...
router.post('/', async function(req, res) {  
  ...
  enroll();
  res.status(201).send("Successfully Register Admin!");
});
...
router.delete('/ddo', async function(req, res) {
  ...
  res.status(204).send("Successfully invoke transaction");
})

router.post('/vc', async function(req, res) {
  const invoke = require("./lib/invoke");
  invoke("registerVC", req.body);
  res.status(201).send("Successfully invoke registerVC transaction");
})
